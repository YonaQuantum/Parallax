...
上述步骤用于启动服务，不代表已满足开放注册条件。首发已选择直接开放邮箱注册；发布前逐项完成 [首发验收](community-launch-plan.md#首发前必须完成的验收)，尤其验证 SMTP 失败恢复、密码找回、退出、限流、管理权限及数据库/上传备份恢复。本次方案没有改变 Compose 或反向代理配置，也没有执行生产验收。
