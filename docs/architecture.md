...
2026-09-07 确认各领域条目先进入站内详情，外部 Signal 也承载图文摘要与成员讨论。建议保留 `/content/[slug]`、新增 `/signals/[id]`，复用阅读与讨论组件。`Content` 与 `IngestedArtifact` 保留各自来源语义。

共用 `DiscussionThread` 是拟议模型，当前 schema 尚未引入。实施时需要关联约束、既有评论回填、回复归属校验和归档保留策略，并同步 migration。首发开放邮箱注册，账号状态、权限与恢复流程需要补齐。完整现状和方案见 [社区融合体验与首发方案](community-launch-plan.md)。
