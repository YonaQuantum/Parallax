...
- `User`：用户、主理人、版主和普通成员
- `AuraIdentity`：AURA 身份证、展示编号、短码、卡面样式和成员主页资料
...
AURA 身份证已经作为用户身份层进入数据库，而不是作为根目录里的静态素材存在。

- `serial`：展示编号，例如 `AURA-0001`
...
种子脚本位于 `packages/db/prisma/seed.ts`，只初始化站点所有者和基础领域时钟。普通注册用户会自动生成新的 AURA 身份证和白卡。真实成员资料不进入开源仓库。

## 认证与邮箱验证

当前认证是自托管数据库 session：

- 注册写入 `User`、生成标准白卡 `AuraIdentity`，再发送邮箱验证邮件。
