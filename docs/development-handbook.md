...
### 开放注册与统一详情的首发缺口（2026-09-07）

维护者已确认直接开放邮箱注册，并让外部 Signal 先进入站内图文详情参与讨论。当前不能把 `Account`、角色、收藏等预留模型视为完整功能，也不能把身份卡状态当作账号封禁。

源码检查发现：邮件失败后未验证账号缺少恢复入口；退出函数未接出；身份卡计数分配存在并发碰撞窗口；密码字符上限与 bcrypt 字节边界不一致；评论写入未校验目标发布状态；成员列表未过滤未验证账号；上传依赖客户端 MIME 并保留原扩展名。完整来源与处理验收见 [社区融合体验与首发方案](community-launch-plan.md#首发前必须完成的验收)。这些事项尚未修复或完成生产验证。

统一详情需要保留外部资料身份、稳定地址与讨论归属。共用讨论线程属于待实施迁移提案，不能直接删除原评论关联或让 Radar 归档级联删除成员讨论。密码、会话、验证 token 与真实用户数据不得出现在日志、测试 fixture 和提交中。
